...
import { AlertTriangle, TrendingUp, Shield, Globe, Plane, Users, Target, Activity } from 'lucide-react';
import { NewsEvent } from '../utils/newsSystem';
import { Aircraft, AircraftStatus, MissionType, Side } from '../types/game';
...
export const NewsModal: React.FC<NewsModalProps> = ({ isOpen, onClose, news, aircrafts, allyAircrafts }) => {
  const friendlyAircraft = (aircrafts || []).filter(a => a.side === Side.FRIENDLY && a.status !== AircraftStatus.DESTROYED);
  const allyAir = (allyAircrafts || []).filter(a => a.side === Side.ALLY && a.status !== AircraftStatus.DESTROYED);

  const activeMissions = friendlyAircraft.filter(a => a.targetId || a.status === AircraftStatus.CRUISE || a.status === AircraftStatus.COMBAT);
  const idleAircraft = friendlyAircraft.filter(a => a.status === AircraftStatus.HANGAR || a.status === AircraftStatus.REST_IN_ACTION);
      ...
      {isOpen && (
        ...
        >
          ...
          >
                  ...
                  <div className="space-y-1">
                    {activeMissions.slice(0, 5).map(ac => (
                      <div key={ac.id} className="flex justify-between items-center p-2 bg-slate-800/50 rounded text-xs">
                        <span className="text-slate-300">{ac.spec.model}</span>
                        <span className={`px-2 py-0.5 rounded text-[10px] ${
                          ac.status === AircraftStatus.COMBAT ? 'bg-red-500/20 text-red-400' :
                          ac.status === AircraftStatus.CRUISE ? 'bg-blue-500/20 text-blue-400' :
            ...
            <button onClick={onClose} className="mt-4 w-full py-2 bg-amber-500/20 border border-amber-500/40 text-amber-400 font-bold rounded hover:bg-amber-500/30 transition-colors">
              DISMISS
            </button>
          </motion.div>
        </motion.div>
