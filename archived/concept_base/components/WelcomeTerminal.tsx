...
const lorebookContent = [
  {
    title: "OPERATION: PURGATÓRIO DE RADAR",
    subtitle: "Phase 16 - STRATOSFEAR Expansion",
    content: "The Cold War never ended. It evolved. In the vacuum left by old superpowers, 15 geopolitical factions now compete for air superiority, resources, and influence. You command a regional air force. Survive. Dominate. Control the skies.",
  },
  {
    title: "THE 15 FACTIONS",
    subtitle: "A New World Order",
    content: "European Coalition (EC), Baltic Naval Strike (BNS), Pan-African Alliance (PAN), Latin American Bloc (LA), Sino-Pacific (SP), Central European (CE), Arabian Defense (AD), United Asian Theater (UAT), Mediterranean Venture (MV), Caucasus Group (CG), Zebu Strategic (ZS), Himalayan Mandate (HM), Indian Frontier (IF), Caribbean Syndicate (CS), Atlantic Vanguard (AV).",
  },
  {
    title: "THE STOCK MARKET",
    subtitle: "War is Economics",
    content: "Every aircraft destroyed impacts faction stock prices. Victory in combat raises your faction's market value. Losses drain it. The war ledger tracks real-time prices of all 15 factions—watch market movements as battles unfold. Economic warfare is as important as air combat.",
  },
  {
    title: "TERRAIN MATTERS",
    subtitle: "Crash Dynamics",
    content: "Where you crash determines the damage cost and pilot survival rate. Urban terrain inflicts 15x damage multiplier but low pilot survival (10%). Desert offers low damage (0.5x) but better survival (65%). Mountains balance cost and risk. The Incident Report ledger records every loss with financial breakdown.",
  },
  {
    title: "AI FACTION PERSONALITIES",
    subtitle: "Predictable Enemies",
    content: "Each hostile faction has a unique personality profile: aggressive attackers (BNS), cautious evaders (CE), balanced operators (EC). The AI uses Q-Learning with faction personality weights to decide actions—engage, notch, crank, evade, retreat, or terrain mask. You can predict and counter each faction's tendencies.",
  },
  {
    title: "YOUR MISSION",
    subtitle: "Command Air Superiority",
    content: "Scramble interceptor flights. Assign patrol, loiter, and strike missions. Manage fuel, ammo, and ECM. Engage hostile aircraft in realistic combat. Build bases, upgrade radar and hangars. Lead your faction to economic and military victory in the geopolitical skies.",
  },
];
...
export const WelcomeTerminal: React.FC<WelcomeTerminalProps> = ({ isOpen, onClose }) => {
  ...
  const page = lorebookContent[currentPage];
            ...
            <div className="flex items-center justify-between mt-6 pt-4 border-t border-emerald-500/20">
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs text-emerald-500/60">
                  {currentPage + 1} / {lorebookContent.length}
                </span>
                {currentPage === lorebookContent.length - 1 ? (
                  <button
                    onClick={onClose}
