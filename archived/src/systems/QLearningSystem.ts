import { QAction } from '../types/entities';
...
const ACTIONS: QAction[] = ['ENGAGE', 'NOTCH', 'CRANK', 'EVADE', 'RETREAT', 'TERRAIN_MASK'];

const ACTION_PERSONALITY_WEIGHTS: Record<QAction, keyof FactionPersonalityProfile> = {
...
export class QTable {
  ...
  getQ(state: AIState, action: QAction): number {
    const entry = this.getOrCreateEntry(state);
    return entry[ACTIONS.indexOf(action)];
  }

  chooseAction(state: AIState): QAction {
  ...
  update(state: AIState, action: QAction, reward: number, nextState: AIState) {
    const key = this.stateToKey(state);
